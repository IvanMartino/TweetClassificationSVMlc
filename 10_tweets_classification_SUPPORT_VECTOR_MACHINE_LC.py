...
using Support Vector Machine with SGD Classifier
...
def main():
    ...
    n_features=2000 #this could be adjusted later by the algorithm

    vectorizer = CountVectorizer(min_df=20,
                                 max_features=n_features,
                                 lowercase=False)

    m=15000 #number of elements in the traning set
            #
    ...
    print("-----------------------------")
    print("METHOD - MULTI-CLASS SUPPORT VECTOR MACHINE - SGD Classifier")
    print("-----------------------------")
    ...
    model_LC = make_pipeline(StandardScaler(with_mean=False), SGDClassifier(loss='modified_huber', max_iter=50000, tol=1e-6))
    model_LC.fit(X_train, Y_train)

    #clf.predict(X_dev)
    acc=model_LC.score(X_train, Y_train)
    acc_dev=model_LC.score(X_dev, Y_dev)
    print("Accuracy on the training set", acc)
    print("Accuracy on the development set", acc_dev)

    df_score, df_fp, df_pre=compute_accuracies(model_LC,1,screen_names, X_train[:,0:n_features], X_dev[:,0:n_features], Y_train, Y_dev)
